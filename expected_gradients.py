...
class AttributionPriorExplainer(object):
    def __init__(self, background_dataset, batch_size, random_alpha=True,k=1,scale_by_inputs=True, p_null_ref=0):
        self.random_alpha = random_alpha
        self.k = k
        self.scale_by_inputs = scale_by_inputs
        self.batch_size = batch_size
        self.p_null_ref = p_null_ref
    ...
    def _get_ref_batch(self,k=None):
        #return next(iter(self.ref_sampler))[0][0][0].float()
        return next(iter(self.ref_sampler))[0][0].float()
    ...
    def _get_grads(self, samples_input, model, sparse_labels=None):
        ...
        for i in range(self.k):
            ...
            if len(batch_output.size())>1:
                batch_output = torch.mean(batch_output, list(range(1, len(batch_output.size()))))
            model_grads = grad(
    ...
    def shap_values(self, model, input_tensor, sparse_labels=None):
        ...
        if self.ref_set is not None:
            ...
            reference_tensor = torch.roll(input_tensor.unsqueeze(1), 1, 0)
            #hape = reference_tensor.shape
            for i in range(self.k-1):
                reference_tensor = torch.cat((reference_tensor, torch.roll(input_tensor.unsqueeze(1), i+2, 0)), 1)
        for i in range(self.k):
            if random.random()<self.p_null_ref:
                reference_tensor[:,i] = torch.zeros(reference_tensor[:,i].size())
